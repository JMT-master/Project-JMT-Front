         ...
         <div className='detail-contentReview'>
           ...
           <div className='detail-contentReview-content' id='review' style={{ display: tabState.reviewVisible ? 'grid' : 'none' }}>
             <button style={{display: tabState.reviewWriteVisible ? 'none' : 'grid' }} onClick={(e) => {
               e.preventDefault();
               toggleTab('reviewWriteVisible') }}>리뷰 작성하기</button>
             <form onSubmit={handleFormSubmit} className="review-writeBox" style={{border :"1px black", display: tabState.reviewWriteVisible ? 'grid' : 'none' }}>
               <div>리뷰 작성</div>
